...
const CartItem: React.FC<CartItemProps> = ({ item }) => {
  ...
  const handleIncrement = () => {
    if (!item.isFree && item.quantity < item.product.stock) {
      updateQuantity(item.product.product_code, item.quantity + 1);
      if (item.quantity === 5 && item.product.product_name.toLowerCase().includes('coca-cola')) {
        toast({
          title: 'Almost there!',
      ...
      if (item.quantity === 2 && item.product.product_name.toLowerCase().includes('croissant')) {
        toast({
          title: 'Special offer!',
      ...
      <div className="flex-grow">
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start">
          <div className="text-center sm:text-left mb-3 sm:mb-0">
            ...
            {item.isFree && (
              <Badge variant="outline" className="bg-green-100 text-green-600 border-green-200 mt-1">
                Free Item
              </Badge>
            )}
          </div>
          ...
          <div className="flex items-center justify-center sm:justify-end">
            {!item.isFree ? (
              ...
              <div className="text-sm mr-2 bg-green-50 px-2 py-1 rounded-full">×{item.quantity}</div>
            )}
          </div>
        </div>
      </div>
